...
namespace kmx::sat::cdcl
{
    ...
    TEST_CASE("database", "[sat]")
    {
        ...
        const std::array<literal, 2u> original_literals {a_pos, b_pos};
        ...
        const std::array<literal, 2u> learned_literals {b_pos, c_pos};
        ...
        REQUIRE(visited_irredundant.size() == 1u);
        REQUIRE(visited_irredundant[0u] == original_ref);
        ...
        REQUIRE(stats_before.irredundant_count == 1u);
        REQUIRE(stats_before.redundant_count == 1u);
        REQUIRE(stats_before.garbage_count == 1u);
        ...
        REQUIRE(stats_after.redundant_count == 0u);
        REQUIRE(!database.is_garbage(learned_ref));
    ...
    {
        clause::database database;
        const std::array<literal, 2u> literals {literal {variable {41u}, false}, literal {variable {42u}, true}};
