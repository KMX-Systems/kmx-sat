...
namespace kmx::sat::cdcl
{
    ...
    TEST_CASE("storage", "[sat]")
    {
        ...
        const std::array<literal, 2> original_literals {a_pos, b_neg};
        ...
        REQUIRE(original_literals_readback.size() == 2);
        REQUIRE(original_literals_readback[0] == a_pos);
        REQUIRE(original_literals_readback[1] == b_neg);
        ...
        const std::array<literal, 3> learned_literals {a_pos, b_neg, c_pos};
        ...
        REQUIRE(learned_literals_readback.size() == 3);

        // Shrinking truncates the stored literal payload in place.
        storage.shrink_clause(learned_ref, 2);
        const auto shrunk_literals = storage.literals_of(learned_ref);
        REQUIRE(shrunk_literals.size() == 2);
        REQUIRE(shrunk_literals[0] == a_pos);
        REQUIRE(shrunk_literals[1] == b_neg);
        ...
        REQUIRE(relocated_literals.size() == 2);
        REQUIRE(relocated_literals[0] == a_pos);
        REQUIRE(relocated_literals[1] == b_neg);
