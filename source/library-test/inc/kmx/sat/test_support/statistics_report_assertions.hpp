...
namespace kmx::sat::test_support
{
                                                           ...
                                                           const telemetry::solver_statistics::snapshot& snapshot) noexcept
    {
        return parsed.contains("conflicts") && parsed.contains("decisions") && parsed.contains("propagations")
            && parsed.contains("restarts") && parsed.contains("learned_clauses")
            && parsed.at("conflicts") == snapshot.conflicts && parsed.at("decisions") == snapshot.decisions
            && parsed.at("propagations") == snapshot.propagations && parsed.at("restarts") == snapshot.restarts
            && parsed.at("learned_clauses") == snapshot.learned_clauses
            && parsed.contains("learned_clause_glue_total")
            && parsed.contains("learned_clause_glue_samples")
            && parsed.at("learned_clause_glue_total") == snapshot.learned_clause_glue_total
            && parsed.at("learned_clause_glue_samples") == snapshot.learned_clause_glue_samples
            && parsed.contains("reduction_passes") && parsed.contains("reduced_clauses") && parsed.contains("deleted_clauses")
            && parsed.at("reduction_passes") == snapshot.reduction_passes
            && parsed.at("reduced_clauses") == snapshot.reduced_clauses
    ...
    inline bool compact_statistics_report_exactly_matches_snapshot(const statistics_report_map& parsed,
                                                                   const telemetry::solver_statistics::snapshot& snapshot) noexcept
    {
        return parsed.size() == compact_statistics_report_field_count
            && compact_statistics_report_matches_snapshot(parsed, snapshot);
    }

    inline bool verbose_statistics_report_exactly_matches_snapshot(const statistics_report_map& parsed,
                                                                   const telemetry::solver_statistics::snapshot& snapshot) noexcept
    {
        return parsed.size() == verbose_statistics_report_field_count
            && verbose_statistics_report_matches_snapshot(parsed, snapshot);
    }
    ...
    {
        return parsed.size() == verbose_statistics_report_field_count
            && parsed.contains("conflicts") && parsed.contains("decisions") && parsed.contains("propagations")
            && parsed.contains("restarts") && parsed.contains("learned_clauses")
            && parsed.contains("learned_clause_glue_total") && parsed.contains("learned_clause_glue_samples")
            && parsed.contains("terminate_callback_calls")
                                                                       ...
                                                                       const telemetry::solver_statistics::snapshot& delta) noexcept
    {
        ...
        return saturating_delta(before.at("conflicts"), after.at("conflicts")) == delta.conflicts
            ...
            && saturating_delta(before.at("configuration_updates"), after.at("configuration_updates"))
                == delta.configuration_updates;
    }
}
