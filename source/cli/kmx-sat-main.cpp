...
int main(int argc, char* argv[])
{
    ...
    for (int index = 2; index < argc; ++index) {
        ...
        if (option == "--assume" && index + 1 < argc) {
            ...
            request.assumptions.emplace_back(
                kmx::sat::variable(static_cast<kmx::sat::variable::index_t>(std::abs(value))), value < 0);
    ...
    std::cout << "c kmx-stats conflicts=" << statistics.conflicts << " decisions=" << statistics.decisions
              ...
              << " learned_clause_glue_total=" << statistics.learned_clause_glue_total
              << " learned_clause_glue_samples=" << statistics.learned_clause_glue_samples
              << " reduction_passes=" << statistics.reduction_passes
              << " reduced_clauses=" << statistics.reduced_clauses
              << " deleted_clauses=" << statistics.deleted_clauses
